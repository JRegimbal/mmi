...
namespace Interpol {
    namespace Gaussian {
        ...
        MatrixXd InterpolationMatrix(const std::vector<ControlPoint>&  points, double shape) {
        ...
        VectorXd Interpolate(const MatrixXd& weightMatrix, const Vector2d& visual, const std::vector<ControlPoint>& points, double shape) {
            // Weight matrix goes column by column for each parameter. I.e., column 0 are weights for parameter 0.
            ...
            for (size_t param= 0; param < weightMatrix.cols(); param++) {
                ...
                for (size_t point = 0; point < points.size(); point++) {
                    if (param == 0) {
                        rbfresult[point] = rbf((points.at(point).visual - visual).norm(), shape);
                    }
